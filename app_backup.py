...
def get_ai_analysis(_conn: snowflake.connector.SnowflakeConnection, test_id: str, valid_id: str) -> str:
    """
    Gets AI analysis by calling the GET_AI_ANALYSIS stored procedure in Snowflake.

    Args:
        _conn: An active Snowflake connection object.
        test_id: The SOURCE_PKEY of the test customer record.
        valid_id: The ID of the valid customer record.

    Returns:
        A formatted markdown string with the AI analysis, or an error message.
    """
    try:
        # The 'with' statement ensures the cursor is automatically closed
        with _conn.cursor() as cursor:
            # The SQL to call the stored procedure with placeholders for security
            sql = "CALL GET_AI_ANALYSIS(%s, %s)"

            # Execute the call, passing the arguments as a tuple
            cursor.execute(sql, (test_id, valid_id))

            # A stored procedure that returns a value is treated like a query.
            # We fetch the single row and single column result.
            result = cursor.fetchone()

            if result:
                return result[0]  # The analysis string is the first column of the result
            else:
                return "Error: Stored procedure did not return a result."

    except Exception as e:
        # You would typically log the error here
        # import logging
        # logging.error(f"Error calling GET_AI_ANALYSIS SP: {e}")
        return f"An error occurred: {str(e)}"
def update_test_record(_conn, record_id: str, updates: dict) -> bool:
    """Update test record by calling a Snowflake stored procedure."""
    try:
        with _conn.cursor() as cursor:
            # The Python connector handles converting the dict to a JSON string for the OBJECT param
            sql = "CALL UPDATE_TEST_RECORD(%s, PARSE_JSON(%s))"
            cursor.execute(sql, (record_id, json.dumps(updates)))
            result = cursor.fetchone()[0]
            print(result) # e.g., "SUCCESS: Record 123 updated."
...
def get_account_url():
    """Get the Snowflake account URL from the connection config"""
    try:
        connections_path = os.path.expanduser("~/.snowflake/connections.toml")
        if os.path.exists(connections_path):
            with open(connections_path, 'r') as f:
        ...
        return os.getenv('SNOWFLAKE_ACCOUNT_URL')
    except:
        return None
...
def get_auth_token():
    ...
    try:
        ...
        # For demo, we'll create a simple JWT token approach
        # This would need proper implementation in production
        st.error("Authentication token not available. Please set SNOWFLAKE_TOKEN environment variable.")
        return None
    except Exception as e:
        st.error(f"Error getting auth token: {str(e)}")
        return None

def get_predefined_response(user_message: str) -> Optional[Dict]:
    """Get predefined responses for common questions - demo implementation"""
    ...
    if "exact match" in message_lower:
        ...
        return {
            ...
            SELECT 
                ...
                cmr.SIMILARITY_SCORE * 100 as similarity_percentage,
                cmr.VALID_ID as matched_valid_customer_id,
                cmr.VALID_CUSTOMER_FULL_DETAIL as matched_customer_details
            FROM TEST_MATCHES t
            JOIN CUSTOMER_MATCH_RESULTS cmr ON t.SOURCE_PKEY = cmr.TEST_ID
    ...
    elif "edit" in message_lower and ("test_match" in message_lower or "customer" in message_lower):
        return {
            "response": "I can help you edit a test_match record! Please see the edit form below:"
        }
    elif "why" in message_lower and ("different" in message_lower or "records" in message_lower):
        return {
            "response": "To analyze why records are different, I can use AI analysis. Please specify which test customer you'd like me to analyze, or select a customer from the Data View tab to see the AI analysis."
        }
...
def call_cortex_agent(messages: List[Dict], _conn) -> Dict:
    ...
    try:
        ...
        request_body = {
            "model": "llama3.3-70b",
            "response_instruction": "Provide concise, helpful answers. Return text and SQL when applicable.",
            "experimental": {},
            "tools": [
            ...
            if 'delta' in data and isinstance(data['delta'], dict) and 'content' in data['delta']:
                ...
                for item in data['delta'].get('content', []):
                    if item.get('type') == 'text' and item.get('text'):
                        texts_accum.append(item['text'])
                if texts_accum:
                    return {"response": "".join(texts_accum)}

        return {"response": "Your request was sent to the agent. No textual answer was returned."}
    except Exception as e:
...
def update_test_record_via_sql(_conn, record_id: str, updates: Dict) -> bool:
    """Update test record using SQL - called from chatbot"""
    try:
        cursor = _conn.cursor()
        ...
        if current_record:
            # Apply updates to create new full detail
    ...
    if role == 'user':
        ...
        with st.chat_message("assistant"):
            ...
            if "edit" in content.lower() and "test_match" in content.lower():
                ...
                # Try to extract customer ID and show matches
...
def show_inline_edit_form(_conn):
    ...
    with st.expander("📝 Edit Test Customer Record", expanded=True):
        ...
        # Load test customers for selection
        try:
...
def show_inline_top_matches(content: str, _conn):
    ...
    # Try to extract customer ID from content
    import re
    
    # Look for patterns like TEST_001, SOURCE_PKEY = abc, etc.
    patterns = [
        r'TEST_\w+',
        r'source_pkey\s*=\s*(\w+)',
        r'customer\s*(\w+)',
    ...
    if customer_id:
        with st.expander(f"🎯 Top 5 Matches for {customer_id}", expanded=True):
            try:
                # Use existing function to get top matches
...
def process_chat_input(user_input: str, _conn) -> str:
    ...
    try:
        ...
        # Prepare messages for Cortex Agent
...
def main():
    ...
    # Main content tabs
    tab_data, tab_chat = st.tabs(["📋 Data View", "💬 Chat View"])
    ...
    with tab_chat:
        st.header("💬 Customer Matching Assistant")
